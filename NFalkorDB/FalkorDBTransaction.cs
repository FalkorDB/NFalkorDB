...
{
    /// <summary>
    /// Allows for executing a series of FalkorDB queries as a single unit.
    /// </summary>
    public class FalkorDBTransaction
    {
        ...
        /// <summary>
        /// Execute a FalkorDB query with parameters.
        /// </summary>
        ...
        {
            _graphCaches.PutIfAbsent(graphId, new GraphCache(graphId, _falkordb));

            _pendingTasks.Add(new TransactionResult(graphId, _transaction.ExecuteAsync(Command.QUERY, graphId, query, FalkorDB.CompactQueryFlag)));
        ...
        public ValueTask CallProcedureAsync(string graphId, string procedure) =>
            CallProcedureAsync(graphId, procedure, Enumerable.Empty<string>(), FalkorDB.EmptyKwargsDictionary);
